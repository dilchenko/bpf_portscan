...
# Note: on a 2 core VM on my iMac, I can process about 30 items/secon (1373 items in IP registry took 45 seconds)
# as it stands, something like a SYN flood will overwhlem the queue
...
echo "Starting expiration of Portrscan IP registry at ${ktime_current} with cutoff ${ktime_cutoff}"
...
echo "${current_registry_keys_hex}" | parallel -j+32 "./portscan_process_entry.sh {} '${ktime_cutoff}'"
...
#
# Intenally, in the map, '11' would be stored as
...
echo "Finished expiration of Portrscan IP registry at $(cat /proc/uptime | awk '{ print $1}')"
echo "----"
echo ""